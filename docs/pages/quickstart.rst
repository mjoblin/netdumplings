...
2. Start a sniffer kitchen ``nd-sniff``
3. Start a dumpling eater ``nd-print``
...
writing your own **dumpling chefs** in Python (by subclassing
:class:`DumplingChef`) which you register with one or more running instances of
``nd-sniff``.

You'll also write your own **dumpling eaters** (perhaps using the
:class:`DumplingEater` helper class that comes with netdumplings). Dumpling
eaters are just WebSocket clients which receive JSON dumpling payloads so you
can also write them in any other language.
